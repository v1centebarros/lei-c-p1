...
bool cbLabHandler::startElement(const QString &, const QString &, const QString & qName, const QXmlAttributes & attr)
{
    /* process begin tag */
    ...
    else if (tag == "Wall" && lab != 0)
    {
        ...
        const QString & height = attr.value(QString("Height"));
        if (!height.isNull())
            wall->setHeight(height.toDouble());
    }
    else if (tag == "Beacon" && lab != 0)
    {
        ...
        const QString & height = attr.value(QString("Height"));
        if (!height.isNull())
            beacon->setHeight(height.toDouble());
    }
    else if (tag == "Target" && lab != 0)
    {
        ...
        if (!radius.isNull())
            target->setRadius(radius.toDouble());
    }
    ...
    else if (tag == "Row" && lab != 0)
    {
        ...
        int horWallStartCol = 0, horWallEndCol;
        while (!spec->isNull())
        {
            if (row % 2 == 0)
            {                   // only vertical walls are allowed here
                if (spec->toLatin1() == '|')
                {
                    wall = new cbWall;
                    wall->addCorner(((col + 1) / 3.0 - PATHWALLWIDTH) * PATHCUBESIZE,
                            (row * 0.5 + PATHWALLGAP) * PATHCUBESIZE);
                    wall->addCorner(((col + 1) / 3.0 + PATHWALLWIDTH) * PATHCUBESIZE,
                            (row * 0.5 + PATHWALLGAP) * PATHCUBESIZE);
                    wall->addCorner(((col + 1) / 3.0 + PATHWALLWIDTH) * PATHCUBESIZE,
                            (row * 0.5 + 1.0 - PATHWALLGAP) * PATHCUBESIZE);
                    wall->addCorner(((col + 1) / 3.0 - PATHWALLWIDTH) * PATHCUBESIZE,
                            (row * 0.5 + 1.0 - PATHWALLGAP) * PATHCUBESIZE);
            ...
            {                   // only horizontal walls are allowed at odd rows
                if (col % 3 == 0)
                {               // if there is a wall at this collumn then there must also be a wall in the next one
                    ...
                    if ((spec->toLatin1() == ' ' && inHorizontalWall) || (spec->toLatin1() == '-' && col == 39))
                    {
                        ...
                        wall->addCorner((horWallStartCol / 3.0 + PATHWALLGAP) * PATHCUBESIZE,
                                ((row + 1) * 0.5 - PATHWALLWIDTH) * PATHCUBESIZE);
                        wall->addCorner((horWallEndCol / 3.0 + 1.0 - PATHWALLGAP) * PATHCUBESIZE,
                                ((row + 1) * 0.5 - PATHWALLWIDTH) * PATHCUBESIZE);
                        wall->addCorner((horWallEndCol / 3.0 + 1.0 - PATHWALLGAP) * PATHCUBESIZE,
                                ((row + 1) * 0.5 + PATHWALLWIDTH) * PATHCUBESIZE);
                        wall->addCorner((horWallStartCol / 3.0 + PATHWALLGAP) * PATHCUBESIZE,
